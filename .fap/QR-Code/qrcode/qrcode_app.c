...
#define TAG "qrcode"
#define QRCODE_FOLDER "/ext/apps_data/QR-Codes"//ANY_PATH("qrcodes")
...
int32_t qrcode_app(void* p) {
    ...
    if(!storage_simply_mkdir(storage, QRCODE_FOLDER)) {
        FURI_LOG_E(TAG, "Could not create folder %s", QRCODE_FOLDER);
    }
    furi_record_close(RECORD_STORAGE);
    ...
    do {
        if (p && strlen(p)) {
            ...
            furi_string_set(file_path, "QR-Codes");
        } else {
            furi_string_set(file_path, QRCODE_FOLDER);

            DialogsFileBrowserOptions browser_options;
            dialog_file_browser_set_basic_options(&browser_options, QRCODE_EXTENSION, &I_qrcode_10px);
